...
def simulate_dynamics(init_pos, init_vel, step_size, num_steps, potential_fn):
    ...
    for _ in xrange(num_steps - 1):
        ...
        # var_pos.data.copy_(new_pos)
        potential = potential_fn(var_pos).sum(0)
        potential.backward()
        # print(var_pos.grad.data)
...
#     def __init__(self, potential_fn, step_size, target_accept_rate, num_steps)
...
def sampler_on_nd_gaussian(burnin, num_chains, num_samples, dim):
    ...
    def gaussian_energy(x):
        ...
        cuda_mu = Variable(cuda_mu.view(1, -1).expand(x.size()[0], cuda_mu.size()[-1]),
                           requires_grad=False)
